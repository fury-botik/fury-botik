...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("879923560284557333").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("879923560284557333").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('879923560284557333').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("894135009705865247").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("894134487120764929").setName(`На сервере: ${members}`); // свой айди войса
}

var interval = setInterval(function () { change(); }, 20000  ); // время обновления в миллисекундах

client.login('procces.env.BOT_TOKEN') // токен вашего бота
